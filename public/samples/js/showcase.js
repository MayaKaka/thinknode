		...
		init: function(div, cvs, ct, $panel, $fps) {
			var ticker = new ct.Ticker(),
				...
				clock = new ct.DisplayObject({
					renderMode: 1, x: 400-100, y: 240-100, width: 200, height: 200
				}),
		...
		init: function(div, cvs, ct, $panel, $fps) {
			var ticker = new ct.Ticker(),
				...
				board = new ct.DisplayObject({
					renderMode: 1, width: 800, height: 480
				}),
				bg = new ct.Shape({
					renderMode: 1,
			...
			board.on('mousedown', function(e) {
				x = e.offsetX;
				y = e.offsetY;
				color = $control.find('.fillColor').val();
				lineW = parseFloat($control.find('.lineWidth').val());
			...
			var $control = $([
				'<div class="op-show-control-table">',
					'<table>',
						'<tr><td>fillColor</td><td><input type="color" class="fillColor" value="#000000"></td></tr>',
						'<tr><td>lineWidth</td><td><input type="number" value="5" class="lineWidth"></td></tr>',
					'</table>',		
				'</div>'].join(''));
